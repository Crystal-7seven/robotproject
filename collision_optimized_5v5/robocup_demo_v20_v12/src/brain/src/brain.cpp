...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<double>("strategy.cooperation.teammate_collision_width", 0.65);
    declare_parameter<double>("strategy.cooperation.teammate_emergency_stop_distance", 0.75);
    declare_parameter<double>("strategy.cooperation.teammate_yield_speed_limit", 0.25);
...
void Brain::handleSpecialStates() {
    ...
    if (gameState == "SET" && isKickoffSide && !tree->getEntry<bool>("gc_play_stopped")) {
...
void Brain::handleCooperation() {
    ...
    if (!isAgentMode && switchRole) {
        bool hasGoalie = data->penalty[selfIdx] == PENALTY_NONE
            && tree->getEntry<string>("player_role") == "goal_keeper";
        for (int i = 0; i < playerCount; i++) {
            if (i != selfIdx && data->tmStatus[i].isAlive && data->tmStatus[i].role == "goal_keeper") {
                hasGoalie = true;
                break;
            }
        }

        // 没有守门员时，由当前存活队员中 ID 最大者接替，适配 3v3/5v5。
        if (!hasGoalie) {
            int lastAliveId = -1;
            ...
            if (lastAliveId == selfId) {
                tree->setEntry<string>("player_role", "goal_keeper");
                log_("No goalie on field, becoming goalie");
    ...
    // Deterministic ball-control arbitration.
    // The previous implementation only compared cost when the best cost was
    // below a threshold.  When every robot was far from the ball, all robots
    // could therefore remain lead and rush the same target.  We now choose
    // exactly one candidate using (cost, player_id), with player_id breaking
    // near-equal costs so every robot reaches the same decision.
    ...
    if (data->tmImAlive && selfHasBallInfo) {
        bestPlayerId = selfId;
    ...
    for (int i = 0; i < aliveTmIdxs.size(); i++) {
        ...
        // A teammate that has neither a current detection nor a remembered
        // ball position must not participate in the claim election.
        const bool teammateHasBallInfo = tmStatus.ballDetected || tmStatus.ballLocationKnown;
        if (!teammateHasBallInfo) continue;
    ...
    if (selfHasBallInfo) {
        ...
        for (int i = 0; i < aliveTmIdxs.size(); i++) {
            int tmIdx = aliveTmIdxs[i];
            auto tmStatus = data->tmStatus[tmIdx];
            if (!(tmStatus.ballDetected || tmStatus.ballLocationKnown)) continue;
    ...
    data->tmImLead = selfHasBallInfo && bestPlayerId == selfId;
    tree->setEntry<bool>("is_lead", data->tmImLead);
    log_(format("bestPlayer: %d, tmMinCost: %.1f, myCost: %.1f, myCostRank: %d, myStrikerIDRank: %d, lead: %d",
        bestPlayerId, tmMinCost, data->tmMyCost, myCostRank, myStrikerIDRank, data->tmImLead));
...
void Brain::updateBallVelocity()
{
    ...
    if (!data->ballVelInitialized) {
        data->ballVelLastTime = now;
        data->ballVelLastPos = data->ball.posToField;
        data->ballVelInitialized = true;
        return;
    }

    const double dt = (now - data->ballVelLastTime).seconds();
    if (dt > 0.01 && dt < 1.0) {
        const double rawVelX = (data->ball.posToField.x - data->ballVelLastPos.x) / dt;
        const double rawVelY = (data->ball.posToField.y - data->ballVelLastPos.y) / dt;
        constexpr double alpha = 0.3;
        data->ballVelX = data->ballVelX * (1.0 - alpha) + rawVelX * alpha;
        data->ballVelY = data->ballVelY * (1.0 - alpha) + rawVelY * alpha;

        // 视觉跳点不应直接驱动守门员大幅预判。
        const double speed = norm(data->ballVelX, data->ballVelY);
        if (speed > 8.0) {
            const double scale = 8.0 / speed;
            data->ballVelX *= scale;
            data->ballVelY *= scale;
        }
    }
...
void Brain::updateKickoffMemory() {
    ...
    auto ballMoved = [=]() {
        ...
        double posChange = norm(data->ball.posToRobot.x - ballPos.x, data->ball.posToRobot.y 
                                - ballPos.y);
    ...
    const int setPlay = tree->getEntry<int>("gc_set_play");
    const bool opponentSetPlay =
        setPlay != 0 && !tree->getEntry<bool>("gc_is_sub_state_kickoff_side");
    const bool isWaitingForKickoffPreparation = (
    ...
    const bool isNewOpponentSetPlay =
        protocolVersion >= 20 && opponentSetPlay && !opponentSetPlayLatched;
    const bool isV20OpponentSetPlayFinished =
        protocolVersion >= 20 && !opponentSetPlay && opponentSetPlayLatched;
    if (isWaitingForKickoffPreparation
    ...
    if (isV20OpponentSetPlayFinished) {
        ...
        const bool isV20PrimaryKickoff =
            waitingForOpponentPrimaryKickoff && protocolVersion >= 20;
...
rclcpp::Time Brain::timePointFromHeader(std_msgs::msg::Header header) {
    ...
    auto messageTime = rclcpp::Time(sec, nanosec, RCL_ROS_TIME);
    auto receiveTime = this->get_clock()->now();

    // Some camera drivers publish an old capture timestamp after buffering a
    // frame. Feeding that timestamp into ball memory makes a fresh ball look
    // stale (observed as roughly 1.8-1.9 seconds of delay). Use reception time
    // when the timestamp is too far from the current ROS time.
    if (messageTime.get_clock_type() == receiveTime.get_clock_type()) {
        const double offsetMs = std::abs((receiveTime - messageTime).nanoseconds()) / 1e6;
        if (offsetMs > 250.0) {
            RCLCPP_WARN_THROTTLE(
                this->get_logger(), *this->get_clock(), 5000,
                "Vision message timestamp differs from receive time by %.0f ms; using receive time",
                offsetMs);
            return receiveTime;
        }
    }
...
void Brain::gameControlCallback(const game_controller_interface::msg::GameControlData &msg)
{
    ...
    data->timeLastGamecontrolMsg = get_clock()->now();
    tree->setEntry<bool>("gc_play_stopped", msg.stopped != 0);
    ...
    data->secondaryTime = static_cast<int>(msg.secondary_time);
    communication->setGameControllerProtocolVersion(static_cast<int>(msg.version));
    ...
    // game_phase / set_play 替代旧 secondary_state
    ...
    const bool isSubStateKickOffSide =
        (gameSubStateType == "FREE_KICK")
        && (static_cast<int>(msg.kicking_team) == teamId);
    string gameSubState = "NONE";
    if (static_cast<int>(msg.version) >= 20) {
        // ===== 现有 v20 逻辑原样保留，不要改语义 =====
        ...
        // ===== 仅 HL v12 =====
        if (gameSubStateType == "FREE_KICK") {
            switch (static_cast<int>(msg.hl_set_play_stage)) {
                case 0: gameSubState = "GET_READY"; break;
                case 1: gameSubState = "SET"; break;
                case 2: gameSubState = "STOP"; break;
                default: gameSubState = "STOP"; break;
            }
            if (isSubStateKickOffSide && msg.hl_set_play_stage == 2) {
                data->isFreekickKickingOff = true;
                data->freekickKickoffStartTime = get_clock()->now();
            }
        }
    ...
    // 只统计当前组别的球员，不能把 GameController 未使用的槽位算进 3v3。
    for (int i = 0; i < playerCount; i++) {
        data->penalty[i] = static_cast<int>(myTeamInfo.players[i].penalty);
        
        if (static_cast<int>(myTeamInfo.players[i].penalty) == PENALTY_SENT_OFF) {
        ...
        if (static_cast<int>(oppoTeamInfo.players[i].penalty) == PENALTY_SENT_OFF) {
...
double Brain::distToObstacle(double angle) {
    ...
    // Treat communicating teammates as dynamic obstacles as well.  The
    // depth grid cannot reliably see a teammate that is outside the current
    // camera view, while the team packet already contains its field pose.
    // Inflate the lateral clearance by the configured teammate width so the
    // existing safe-direction search can route around it.
