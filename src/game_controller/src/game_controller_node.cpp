...
static_assert(sizeof(HlRoboCupGameControlData) == 688, "Unexpected HL v12 packet size");

constexpr size_t kGameControllerHeaderSize = 4;
constexpr size_t kMaxGameControllerPacketSize = 1024;
...
void GameControllerNode::spin()
{
    ...
    while (rclcpp::ok())
    {
        ...
        else if (version == GAMECONTROLLER_STRUCT_VERSION_V19)
        {
            if (ret != static_cast<ssize_t>(sizeof(RoboCupGameControlDataV19))){
                RCLCPP_INFO(get_logger(), "v19 packet from %s invalid length=%ld expected=%zu",remote_ip.c_str(), ret, sizeof(RoboCupGameControlDataV19));
                continue;
            }
            ...
            if (ret != static_cast<ssize_t>(sizeof(RoboCupGameControlDataV20))){
                  RCLCPP_INFO(get_logger(), "v20 packet from %s invalid length=%ld expected=%zu",remote_ip.c_str(), ret, sizeof(RoboCupGameControlDataV20));
                continue;
            }
...
void GameControllerNode::handle_packet(
    const HlRoboCupGameControlData &data,
    game_controller_interface::msg::GameControlData &msg)
{
    for (int i = 0; i < 4; i++) msg.header[i] = data.header[i];
    msg.version = static_cast<uint8_t>(data.version);
    msg.packet_number = data.packetNumber;
    msg.players_per_team = data.playersPerTeam;
    msg.competition_type = data.gameType;
    msg.stopped = 0;
    msg.state = data.state;
    msg.first_half = data.firstHalf;
    msg.secs_remaining = static_cast<int16_t>(data.secsRemaining);
    msg.secondary_time = static_cast<int16_t>(data.secondaryTime);
    msg.hl_set_play_stage = static_cast<uint8_t>(data.secondaryStateInfo[1]);
    msg.game_phase = GAME_PHASE_NORMAL;
    msg.set_play = SET_PLAY_NONE;
    switch (data.secondaryState) {
        case STATE2_PENALTYSHOOT: msg.game_phase = GAME_PHASE_PENALTY_SHOOT_OUT; break;
        case STATE2_OVERTIME:     msg.game_phase = GAME_PHASE_EXTRA_TIME; break;
        case STATE2_TIMEOUT:      msg.game_phase = GAME_PHASE_TIMEOUT; break;
        case STATE2_DIRECT_FREEKICK:   msg.set_play = SET_PLAY_DIRECT_FREE_KICK; break;
        case STATE2_INDIRECT_FREEKICK: msg.set_play = SET_PLAY_INDIRECT_FREE_KICK; break;
        case STATE2_PENALTYKICK:       msg.set_play = SET_PLAY_PENALTY_KICK; break;
        case STATE2_CORNER_KICK:       msg.set_play = SET_PLAY_CORNER_KICK; break;
        case STATE2_GOAL_KICK:         msg.set_play = SET_PLAY_GOAL_KICK; break;
        case STATE2_THROW_IN:          msg.set_play = SET_PLAY_THROW_IN; break;
        default: break;
    }
    msg.kicking_team = (msg.set_play != SET_PLAY_NONE)
        ? static_cast<uint8_t>(data.secondaryStateInfo[0])
        : data.kickOffTeam;
    for (int i = 0; i < 2; i++) {
        msg.teams[i].team_number = data.teams[i].teamNumber;
        msg.teams[i].field_player_colour = data.teams[i].fieldPlayerColour;
        msg.teams[i].goalkeeper_colour = 0;
        msg.teams[i].goalkeeper = 0;
        msg.teams[i].score = data.teams[i].score;
        msg.teams[i].penalty_shot = data.teams[i].penaltyShot;
        msg.teams[i].single_shots = data.teams[i].singleShots;
        msg.teams[i].message_budget = 0;
        for (int j = 0; j < MAX_NUM_PLAYERS; j++) {
            if (j < HL_MAX_NUM_PLAYERS) {
                const auto &src = data.teams[i].players[j];
                msg.teams[i].players[j].penalty = src.penalty;
                msg.teams[i].players[j].secs_till_unpenalised = src.secsTillUnpenalised;
                msg.teams[i].players[j].warnings = src.numberOfWarnings;
                msg.teams[i].players[j].cautions = 0;
                if (src.redCardCount > 0) {
                    msg.teams[i].players[j].penalty = PENALTY_SENT_OFF;
                }
                if (src.goalKeeper) {
                    msg.teams[i].goalkeeper = static_cast<uint8_t>(j + 1);
                }
            } else {
                msg.teams[i].players[j].penalty = PENALTY_SUBSTITUTE;
                msg.teams[i].players[j].secs_till_unpenalised = 0;
                msg.teams[i].players[j].warnings = 0;
                msg.teams[i].players[j].cautions = 0;
            }
        }
