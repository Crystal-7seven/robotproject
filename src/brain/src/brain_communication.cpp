...
void BrainCommunication::setGameControllerProtocolVersion(int version) {
    _gc_protocol_version.store(version);
}

void BrainCommunication::unicastToGameController() {
    while (_unicast_gamecontrol_flag.load(std::memory_order_relaxed)){
        const int ver = _gc_protocol_version.load();
        int ret = -1;
        if (ver == HL_GAMECONTROLLER_STRUCT_VERSION) {
            gc_return_data_hl.team = brain->config->get_team_id();
            gc_return_data_hl.player = brain->config->get_player_id();
            gc_return_data_hl.message = GAMECONTROLLER_RETURN_MSG_ALIVE;
            ret = sendto(_gc_send_socket, &gc_return_data_hl, sizeof(gc_return_data_hl),
                         0, (sockaddr *)&_gcsaddr, sizeof(_gcsaddr));
        }
        else{// v19/v20：32 字节回传，填 pose/ball 供裁判机/TCM 显示
            gc_return_data.teamNum = brain->config->get_team_id();
            gc_return_data.playerNum = brain->config->get_player_id();
            gc_return_data.fallen = (brain->data->recoveryState == RobotRecoveryState::HAS_FALLEN) ? 1 : 0;
            // 场地系位姿：米 -> 毫米；theta 仍为弧度
            gc_return_data.pose[0] = static_cast<float>(brain->data->robotPoseToField.x *1000.0);
            gc_return_data.pose[1] = static_cast<float>(brain->data->robotPoseToField.y *1000.0);
            gc_return_data.pose[2] = static_cast<float>(brain->data->robotPoseToField.theta);
            ...
            if (brain->data->ballDetected || brain->tree->getEntry<bool>("ball_location_known")) {
                ...
                gc_return_data.ballAge = static_cast<float>(age);
                // 球相对机器人：米 -> 毫米
                gc_return_data.ball[0] = static_cast<float>(brain->data->ball.posToRobot.x *1000.0);
                gc_return_data.ball[1] = static_cast<float>(brain->data->ball.posToRobot.y *1000.0);
            } else {
                gc_return_data.ballAge = -1.f;
                gc_return_data.ball[0] = 0.f;
                gc_return_data.ball[1] = 0.f;
            }
            ret = sendto(_gc_send_socket, &gc_return_data, sizeof(gc_return_data),
            0, (sockaddr *)&_gcsaddr, sizeof(_gcsaddr));
...
void BrainCommunication::spinCommunicationReceiver() {
    ...
    while (_receive_communication_flag.load(std::memory_order_relaxed)) {
        ...
        if (brain->data->penalty[tmIdx] == PENALTY_SUBSTITUTE) { // Ignore substitute players' information
